                    ...
                    try:
                        ...
                        if cached_peer_id and cached_access_hash is not None:
                            try:
                                target = InputPeerChannel(
                                    int(cached_peer_id), int(cached_access_hash)
                                )
                            except Exception:
                                target = None
                        if target is None:
                            ...
                            try:
                                ...
                                channel_hash = int(
                                    getattr(entity, "access_hash", 0) or 0
                                )
                            except (TypeError, ValueError):
                                channel_id = 0
                                channel_hash = 0
                            if channel_id and channel_hash:
                                repo.update_source_tg_peer(
                                    int(source["id"]), channel_id, channel_hash
                                )
                        ...
                        for message in reversed(candidates):
                            ...
                            new_message_id = repo.upsert_message(
                                ...
                                published_at=msg_date_utc.strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                ),
                                text=text,
                                ...
                                enqueue_ai=monitor_cfg["ai_enabled"]
                                and bool(source.get("ai_enabled")),
                            )
                            if not (
                                monitor_cfg["ai_enabled"]
                                and bool(source.get("ai_enabled"))
                            ):
                                repo.mark_message_no_ai(
                                    new_message_id, _get_default_category_name()
                                )
                            ingested += 1
                            await _process_alerts_for_message(
                                new_message_id, "new_message"
                            )
                    except Exception:
                        continue
